...
class PicChecker():
    ...
    def __init__(self,chars = initChars(),size = (120,24),fontsize = 20,   
                begin = (5,-2),outputType = 'GIF',mode = 'RGB' ,   
                backgroundColor = (255,255,255), foregroundColor = (0,0,255),   
                fonttype = "/Library/Fonts/Microsoft/Arial.ttf",length = 5,jamNum = (2,3),   
                pointBorder = (40,39)):   
        """  
        初始化配置  
        """   
    ...
    def getPicString(self):   
        ...
        salt = ''.join(random.choice(string.ascii_uppercase + string.digits) for x in range(6))
        return salt
      
    def createChecker(self,captcha_no):   
        """  
        usage: createChecker()  
        return: [str,pic]  
        str:对应的字符串  
